# Lettre de motivation — Template adaptable

> **Instructions d'usage :** remplace les `[CROCHETS]` par les infos de l'entreprise visée.
> Garde **1 page maximum** (4 paragraphes, 300 mots).
> Adapte le paragraphe 2 selon que tu vises ESN, DSI CAC40 ou secteur public.

---

## Version standard — Grosses boîtes (ESN / DSI)

**Moussa Zedira**
Argenteuil (95) — 06 XX XX XX XX
moussazedira@gmail.com
LinkedIn : moussa-zedira | Portfolio : [votre-url]

**[Nom de l'entreprise]**
À l'attention du service Recrutement / [Nom du recruteur si connu]
[Adresse]

Argenteuil, le [date]

**Objet : Candidature alternance BTS SIO option SISR — [Référence offre si applicable]**

Madame, Monsieur,

Actuellement en formation **Titre Professionnel Technicien d'Assistance Informatique à Doranco**, je recherche une **alternance pour préparer un BTS SIO option SISR (Solutions d'Infrastructure, Systèmes et Réseaux)** afin de consolider mes compétences en administration système et réseau au sein d'une équipe IT structurée. Votre [entreprise / ESN / DSI] [phrase personnalisée : "réputée pour X" / "dont je suis l'actualité autour de Y" / "qui accompagne des clients comme Z"] correspond exactement à l'environnement dans lequel je souhaite progresser.

Ce qui me différencie d'un profil support classique, c'est la double compétence que je construis : **méthode rigoureuse du support IT** (diagnostic, GLPI, Active Directory, documentation) et **maîtrise de l'IA et de l'automatisation** (Python, Claude, scripts PowerShell). Concrètement, j'ai monté un HomeLab complet — contrôleur de domaine, serveur GLPI, 50 utilisateurs AD simulés, scripts de création masse — et j'y intègre un triage automatique des tickets via API Claude. Cette approche me permet d'aller au-delà de la simple résolution : comprendre les causes, documenter, automatiser.

Mes projets techniques (plateforme cybersécurité CyberDef, plateforme éducative Chapitres en ligne, jeu d'échecs avec IA) démontrent ma rigueur, mon autonomie et ma capacité à mener un projet de bout en bout. Mes certifications Cisco (Cybersecurity, Networking, Python, Linux) valident les fondamentaux. Visibles sur mon portfolio : [URL].

Je serais ravi d'échanger avec vous pour détailler ce que je peux apporter à vos équipes et découvrir vos missions. Je reste à votre disposition pour un entretien.

Cordialement,
**Moussa Zedira**

---

## Phrase d'accroche personnalisée — par type d'entreprise

### ESN (Sopra Steria, Capgemini, CGI, Atos, Accenture, Sogeti...)
> *Votre ESN, reconnue pour accompagner les grands comptes dans leur transformation numérique et pour la diversité des missions proposées aux jeunes talents, correspond exactement à l'environnement où je souhaite construire mon expertise.*

### DSI grand groupe (BNP, Société Générale, L'Oréal, TotalEnergies, SNCF...)
> *Intégrer la DSI de [Nom Groupe] représente pour moi l'opportunité d'évoluer dans un environnement exigeant, au service de milliers d'utilisateurs internes, et de contribuer à des systèmes critiques.*

### Secteur public (ministères, collectivités, hôpitaux, Pôle Emploi...)
> *Servir une structure publique comme [Nom] a du sens pour moi : contribuer à la qualité du service rendu aux agents et aux citoyens, tout en découvrant les enjeux de sécurité et de souveraineté spécifiques à ce secteur.*

### PME / Scale-up tech
> *La culture de [Nom entreprise], où chaque membre de l'équipe IT a un impact direct, correspond à ma façon de travailler : comprendre le besoin, livrer vite, documenter proprement.*

---

## Checklist avant envoi

- [ ] Nom de l'entreprise et du destinataire vérifiés (pas de "Madame, Monsieur" si tu as un nom)
- [ ] Référence de l'offre mentionnée (objet ou 1er paragraphe)
- [ ] Phrase d'accroche personnalisée (au moins 1 détail spécifique à l'entreprise)
- [ ] Rythme d'alternance précisé (ex. 3 semaines entreprise / 1 semaine école)
- [ ] URL portfolio cliquable et à jour
- [ ] Relecture : zéro faute (fais-le relire)
- [ ] Format PDF, nom : `Zedira_Moussa_LM_[NomEntreprise].pdf`
- [ ] Signature scannée ou tapée en gras
